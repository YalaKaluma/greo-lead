...
async def process_onboarding_data(user_number: str, db: Session = Depends(get_db)):
    ...
    if 'tasks_raw' in data:
        tasks = extract_tasks_from_onboarding(data['tasks_raw'])
        quick_win = data.get('quick_win', '')
        
        for task_text in tasks:
            # Mark the quick win with high priority
            is_quick_win = quick_win.lower() in task_text.lower()
    ...
    return {
        ...
        "goal_added": 'first_goal' in data,
        "tasks_added": len(extract_tasks_from_onboarding(data.get('tasks_raw', '')))
    }
