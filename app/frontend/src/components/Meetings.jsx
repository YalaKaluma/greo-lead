    ...
    <div className="mx-auto max-w-6xl p-5 lg:p-10">
      ...
      {meeting.processing_status === 'ready' && <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="space-y-6">
          ...
          <ContextReceipt receipt={meeting.context_receipt} />
        ...
        <aside className="space-y-5">
          ...
          <Section title={t('meetings.contextReview.title')}>
            ...
            <div className="mt-4 space-y-5">
              ...
              <div><p className="text-xs font-semibold uppercase tracking-wide text-cyan-700">2. {t('meetings.contextReview.participants')}</p><div className="mt-2 space-y-1">{meetingResolution.people?.length ? meetingResolution.people.map((person, index) => <p key={`${person.speaker_label}-${index}`} className="rounded-lg bg-cyan-50 px-3 py-2 text-sm text-cyan-950">{resolutionLabel(person, 'name')}</p>) : <p className="rounded-lg bg-cyan-50 px-3 py-2 text-sm text-cyan-950">{t('meetings.contextReview.noneDetected')}</p>}</div>{peopleSuggestions.length > 0 && <div className="mt-2 space-y-2">{peopleSuggestions.map((item) => <ContextSuggestionCard key={item.id} item={item} reviewing={reviewingSuggestion} onReview={reviewSuggestion} />)}</div>}</div>
              <div><p className="text-xs font-semibold uppercase tracking-wide text-violet-700">3. {t('meetings.contextReview.projects')}</p><p className="mt-2 rounded-lg bg-violet-50 px-3 py-2 text-sm text-violet-950">{resolutionLabel(meetingResolution.primary_project, 'name')}</p>{projectSuggestions.length > 0 && <div className="mt-2 space-y-2">{projectSuggestions.map((item) => <ContextSuggestionCard key={item.id} item={item} reviewing={reviewingSuggestion} onReview={reviewSuggestion} />)}</div>}</div>
            </div>
