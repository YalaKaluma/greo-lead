...
function MeetingDetail({ meeting, apiUrl, userNumber, onBack, onChanged }) {
  ...
  const [contextOptions, setContextOptions] = useState({ people: [], goals: [], projects: [] });
  ...
  const matchParticipant = async (participantId, personId) => {
    await fetch(`${apiUrl}/api/meetings/participants/${participantId}`, {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_number: userNumber, person_id: personId ? Number(personId) : null })
    });
    ...
    <div className="mx-auto max-w-6xl p-5 lg:p-10">
      ...
      {meeting.processing_status === 'ready' && <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <aside className="space-y-5">
          <Section title="Overview"><dl className="space-y-3 text-sm"><div><dt className="text-slate-500">Meeting type</dt><dd className="font-medium">{meeting.meeting_type || 'Other'}</dd></div><div><dt className="text-slate-500">Participants</dt><dd className="mt-2 space-y-2">{meeting.participants.length ? meeting.participants.map((participant) => <label key={participant.id} className="block"><span className="mb-1 block text-xs font-medium">{participant.speaker_label || participant.display_name}</span><select value={participant.person_id || ''} onChange={(event) => matchParticipant(participant.id, event.target.value)} className="w-full rounded-lg border border-slate-300 px-2 py-2"><option value="">{participant.display_name}</option>{contextOptions.people.map((person) => <option key={person.id} value={person.id}>{person.title}</option>)}</select></label>) : <span className="font-medium">Not identified</span>}</dd></div></dl></Section>
