...
const STEPS = [
  ['my-goals', 'Your vision', 'Great leadership starts with a clear destination. This page is where Alfred turns your ambition into a primary vision, strategic pillars, and concrete outcomes so you can see how direction connects to progress.'],
  ['todo-list', 'Your first actions', 'Big goals are achieved through small actions repeated consistently. I created a first set of goal-related actions plus general onboarding tasks you should complete next so your operating system is fully set up. I also ran the MTN prioritization so the highest-leverage work appears first.'],
...
export default function OnboardingReveal({ result, userNumber, onNavigate, onFinish }) {
  ...
  const goalTitle = result?.goal?.title || '';
  const pillarTitles = (result?.pillars || []).map((pillar) => pillar?.title).filter(Boolean);
  const topMtnTasks = (result?.mtn?.top_mtn_tasks || []).slice(0, 3);
    ...
    <section className="pointer-events-auto mx-auto max-w-3xl rounded-2xl border border-amber-200 bg-slate-950 p-5 text-white shadow-2xl md:p-6">
      <div className="flex items-start gap-4"><img src="/alfred-logo.png" alt="Alfred" className="h-11 w-11 rounded-full border border-amber-300 object-cover" />
        <div className="min-w-0 flex-1"><p className="text-xs font-semibold uppercase tracking-wider text-amber-300">{index + 1} of {STEPS.length}</p><h2 className="mt-1 text-xl font-semibold">{title}</h2><p className="mt-2 text-sm leading-6 text-slate-200">{body}</p>
          {index === 0 && goalTitle && (
            <div className="mt-4 rounded-xl border border-amber-300/40 bg-amber-300/10 px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-wide text-amber-200">I created this vision for you</p>
              <p className="mt-1 text-base font-semibold text-white">{goalTitle}</p>
              {pillarTitles.length > 0 && (
                <p className="mt-2 text-xs leading-5 text-slate-300">
                  With pillars around {pillarTitles.join(', ')}.
                </p>
              )}
            </div>
          )}
          {index === 1 && topMtnTasks.length > 0 && (
            <div className="mt-4 rounded-xl border border-amber-300/40 bg-amber-300/10 px-4 py-3">
              <p className="text-xs font-semibold uppercase tracking-wide text-amber-200">Top MTN priorities</p>
              <ol className="mt-2 space-y-1 text-sm text-slate-200">
                {topMtnTasks.map((task, taskIndex) => (
                  <li key={task.task_id || task.title} className="flex gap-2">
                    <span className="font-semibold text-amber-200">{taskIndex + 1}.</span>
