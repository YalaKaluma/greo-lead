...
const PERIOD_OPTIONS = [7, 30, 90, 0];

function TrendsWheel({ averages, periodDays }) {
  const { t } = useLanguage();
  ...
  return <div>
    <svg viewBox="0 0 360 360" className="mx-auto w-full max-w-[430px]" role="img" aria-label={t(`journey.trends.wheelAria.${periodDays}`)}>
      ...
      <text x="180" y="194" textAnchor="middle" className="fill-amber-300 text-[11px]">{t(`journey.trends.hub.${periodDays}`)}</text>
    </svg>
...
export function LeadershipTrendsTab({ apiUrl, userNumber }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    axios.get(`${apiUrl}/api/meetings/leadership-trends`, { params: { user_number: userNumber, days: 90 } })
  ...
  const refreshTrends = async () => {
    ...
    setError('');
    try {
      const response = await axios.post(`${apiUrl}/api/meetings/leadership-trends/refresh`, null, { params: { user_number: userNumber, days: selectedDays } });
  ...
  const periodSelector = <select value={selectedDays} onChange={(event) => setSelectedDays(Number(event.target.value))} disabled={refreshing} aria-label={t('journey.trends.timeFocus')} title={t('journey.trends.timeFocus')} className="cursor-pointer rounded-full border-0 bg-slate-100 px-3 py-1.5 text-sm font-semibold text-slate-700 outline-none ring-blue-500 transition hover:bg-slate-200 focus:ring-2 disabled:cursor-not-allowed disabled:opacity-50">
    {PERIOD_OPTIONS.map((days) => <option key={days} value={days}>{t(`journey.trends.average.${days}`)}</option>)}
  </select>;
  ...
  if (!data?.meeting_count) return <div className="rounded-xl border border-slate-200 bg-white p-8 text-center shadow-sm"><h2 className="text-xl font-semibold text-slate-950">{t('journey.trends.title')}</h2><p className="mx-auto mt-2 max-w-2xl text-slate-600">{t('journey.trends.empty')}</p><div className="mt-5 flex flex-wrap items-center justify-center gap-2">{periodSelector}<button type="button" onClick={refreshTrends} disabled={refreshing} className="rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{refreshing ? t('journey.trends.refreshing') : t('journey.trends.refresh')}</button></div></div>;

  return <section className="space-y-6">
    {error && <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">{error}</div>}
    <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-600">{t(`journey.trends.period.${periodDays}`)}</p>
    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><TrendsWheel averages={data.domain_averages || []} periodDays={periodDays} /></div>
