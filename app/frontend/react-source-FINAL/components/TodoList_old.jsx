...
export default function TodoList({ apiUrl, userNumber }) {
  ...
  // Fetch filters (projects and delegates)
  useEffect(() => {
    fetchFilters();
  }, []);
  ...
  useEffect(() => {
    fetchTasks();
  }, [filterType, selectedProject, selectedDelegate]);
  ...
  const fetchFilters = async () => {
    try {
      ...
      // Backend returns {projects: [...], delegates: [...]} directly
      if (response.data) {
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      // Backend returns array directly: [tasks]
  ...
  const toggleTaskComplete = async (taskId) => {
    try {
      ...
      // Remove from UI immediately - don't reload
  ...
  const hasActiveFilters = selectedProject || selectedDelegate || filterType !== 'all';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        {/* Priority Button (Click to Complete) */}
        <button
          onClick={(e) => {
            e.stopPropagation();
            onToggle();
        ...
        {/* Task Content - More space for text */}
        <div className="flex-1 min-w-0">
          <div className="flex flex-col gap-1">
            ...
            {task.notes && (
              <span className="text-sm text-slate-600">{task.notes}</span>
            )}
...
function AddTaskForm({ onAdd, onCancel, projects, delegates }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
      ...
      {/* Row 2: Due Date, Priority, Notes, Add Button */}
      <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
        ...
        <input
          ...
          className="md:col-span-3 px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
