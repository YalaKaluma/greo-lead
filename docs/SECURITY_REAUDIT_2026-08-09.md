...
|---|---|---:|---|
| C-01 Caller-controlled user identity | **Partially resolved** | **High** | Private routers are registered with `require_authenticated_identity`; mismatched query, JSON, path, `X-User-Number`, and `X-User-Id` claims are rejected. Multipart identity fields have explicit authenticated-owner checks, enforced by a regression inventory test. Object-level ownership filters are present in reviewed resource families, but comprehensive two-user coverage and migration to canonical `current_user.id` remain. |
