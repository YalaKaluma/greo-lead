...
def test_cto_director_creates_codex_ready_copilot_finding(tmp_path, monkeypatch):
    ...
    assert review.source_snapshot_json["cto_review_engine"]["provider"] == "github_models"
    assert review.source_snapshot_json["cto_review_engine"]["status"] == "completed"
...
def test_cto_director_records_copilot_auth_error_as_finding(tmp_path, monkeypatch):
    ...
    assert review.source_snapshot_json["cto_review_engine"]["status"] == "unavailable"
