...
def test_mtn_period_stats_keeps_total_but_averages_weekdays_only():
    rows = [
    ...
    assert stats["eligible_weekdays"] == 5
    assert stats["average_score"] == round(sum([21, 22, 23, 24, 27]) / 5, 2)
